...
export function limitNum(n:number, limit:number) {
  const vorzeichen = n < 0 ? -1 : 1;
  let numberMag = Math.abs(n);
  if (numberMag > limit) {
    numberMag = limit;
  }
...
export function range(start:number, len:number, steps:number):number[] {
...
export async function fetchCSV(file:string) {
	...
	let cols:string[][] = new Array(rows.length);
...
export class PerlinNoise {
  ...
  constructor() {
    this.permutation = [
      ...
      101,155,167, 43,172,9,129,22,39,253,19,98,108,110,79,113,224,232,
