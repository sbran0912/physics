...
export class Polygon implements Shape {
  ...
  update() {
    this.basic.velocity    = std.addVector(this.basic.velocity, this.basic.accel);
    this.basic.angVelocity += this.basic.angAccel;

    this.basic.velocity    = std.multVector(this.basic.velocity, 0.9995);
    this.basic.location    = std.addVector(this.basic.location, this.basic.velocity);
    for (let i = 0; i < this.vertices.length; i++) {
      this.vertices[i] = std.addVector(this.vertices[i], this.basic.velocity);
    }
  ...
  applyGravity(gravity: std.Vector) {
    if (this.basic.mass < INF) {
      if (!this.basic.isGrounded) {
        ...
        if (this.basic.velocity.mag() < 0.5 &&
...
export class Circle implements Shape {
  ...
  update() {
    ...
    this.basic.velocity    = std.multVector(this.basic.velocity, 0.9995);
    this.basic.location    = std.addVector(this.basic.location, this.basic.velocity);
    this.orientation       = std.addVector(this.orientation, this.basic.velocity);
...
function projectPolygon(vertices: std.Vector[], axis: std.Vector): [number, number] {
...
function findContactPoints(verticesA: std.Vector[], verticesB: std.Vector[]): std.Vector[] {
...
function findReferenceEdge(vertices: std.Vector[], normal: std.Vector): [std.Vector, std.Vector] {
  let bestDot = -INF;
...
function projectPointOntoEdge(p: std.Vector, a: std.Vector, b: std.Vector): std.Vector {
...
function transferContactsToA(contacts: std.Vector[], verticesA: std.Vector[], normal: std.Vector): std.Vector[] {
...
function resetPolyPositionsBasedOnMass(polyA: Polygon, polyB: Polygon, mtv: std.Vector) {
...
function resetCirclePolyPositionsBasedOnMass(poly: Polygon, circle: Circle, mtv: std.Vector) {
...
function resetCirclePositionsBasedOnMass(cA: Circle, cB: Circle, mtv: std.Vector) {
...
type RelVectors = { rAP_perp: std.Vector; rBP_perp: std.Vector; vGesamtA: std.Vector; velocity_AB: std.Vector };

function calcPolyRelVectors(polyA: Polygon, polyB: Polygon, cp: std.Vector): RelVectors {
  ...
  return { rAP_perp, rBP_perp, vGesamtA, velocity_AB: std.subVector(vGesamtA, vGesamtB) };
}

function calcCirclePolyRelVectors(poly: Polygon, circle: Circle, cp: std.Vector): RelVectors {
...
function calcCircleRelVectors(cA: Circle, cB: Circle, mtv: std.Vector): RelVectors {
...
function isCircleGrounded(angVel: number, velocity_AB: std.Vector, mtv: std.Vector): boolean {
  if (velocity_AB.mag() > 0.5)       
    return false;
...
function isPolyGrounded(contacts: std.Vector[], vGesamtA: std.Vector, velocity_AB: std.Vector, mtv: std.Vector): boolean {
...
function isCenterOfMassSupported(location: std.Vector, contacts: std.Vector[], normal: std.Vector): boolean {
...
function calcFrictionVector(mtv: std.Vector, velocity_AB: std.Vector): std.Vector {
...
function calcPolyImpulse(
  ...
  mtv: std.Vector, rAP_perp: std.Vector, rBP_perp: std.Vector, velocity_AB: std.Vector,
  e: number,
): number {
  const jNum   = std.dotProduct(std.multVector(velocity_AB, -(1 + e)), mtv);
  const jLinear = std.dotProduct(mtv, std.multVector(mtv, 1 / polyA.basic.mass + 1 / polyB.basic.mass));
  const jAng    = Math.pow(std.dotProduct(rAP_perp, mtv), 2) / polyA.basic.inertia +
...
function calcCirclePolyImpulse(
  ...
  mtv: std.Vector, rAP_perp: std.Vector, velocity_AB: std.Vector,
  e: number,
): number {
  const jNum    = std.dotProduct(std.multVector(velocity_AB, -(1 + e)), mtv);
  const jLinear = std.dotProduct(mtv, std.multVector(mtv, 1 / poly.basic.mass + 1 / circle.basic.mass));
...
function calcCircleImpulse(
  cA: Circle, cB: Circle,
  mtv: std.Vector, velocity_AB: std.Vector,
  e: number,
): number {
  const jNum    = std.dotProduct(std.multVector(velocity_AB, -(1 + e)), mtv);
  const jLinear = std.dotProduct(mtv, std.multVector(mtv, 1 / cA.basic.mass + 1 / cB.basic.mass));
  return jNum / jLinear;
}

type Forces = { forceA: std.Vector; angForceA: number; forceB: std.Vector; angForceB: number };
...
function calcPolyCollisionForces(
  ...
  const forceA    = std.addVector(std.multVector(mtv, j / polyA.basic.mass), std.multVector(t, f * -j / polyA.basic.mass));
  const angForceA = std.dotProduct(rAP_perp, std.addVector(std.multVector(mtv, j / polyA.basic.inertia), std.multVector(t, f * -j / polyA.basic.inertia)));
  const forceB    = std.addVector(std.multVector(mtv, -j / polyB.basic.mass), std.multVector(t, f * j / polyB.basic.mass));
  const angForceB = std.dotProduct(rBP_perp, std.addVector(std.multVector(mtv, -j / polyB.basic.inertia), std.multVector(t, f * j / polyB.basic.inertia)));
...
function calcCirclePolyCollisionForces(
  ...
  const forceA    = std.addVector(std.multVector(mtv, j / poly.basic.mass), std.multVector(t, f * -j / poly.basic.mass));
  const angForceA = std.dotProduct(rAP_perp, std.addVector(std.multVector(mtv, j / poly.basic.inertia), std.multVector(t, f * -j / poly.basic.inertia)));
  const forceB    = std.addVector(std.multVector(mtv, -j / circle.basic.mass), std.multVector(t, f * j / circle.basic.mass));
  const angForceB = std.dotProduct(rBP_perp, std.multVector(t, f * j / circle.basic.inertia));
  return { forceA, angForceA, forceB, angForceB };
}
...
function calcCircleCollisionForces(
  ...
  const forceA    = std.addVector(std.multVector(mtv, j / cA.basic.mass), std.multVector(t, f * j / cA.basic.mass));
  const angForceA = std.dotProduct(rAP_perp, std.multVector(t, f * j / cA.basic.inertia));
  const forceB    = std.addVector(std.multVector(mtv, -j / cB.basic.mass), std.multVector(t, f * -j / cB.basic.mass));
  const angForceB = std.dotProduct(rBP_perp, std.multVector(t, f * -j / cB.basic.inertia));
  return { forceA, angForceA, forceB, angForceB };
}

// ─── Kollisionserkennung ─────────────────────────────────────────────────────

type CollResult = { isColliding: boolean; mtv: std.Vector; contacts: std.Vector[] };
const NO_COLL: CollResult = { isColliding: false, mtv: new std.Vector(0, 0), contacts: [] };
...
export function detectCollisionPoly(polyA: Polygon, polyB: Polygon): CollResult {
  ...
  for (const axis of axes) {
    ...
    if (overlap < smallestOverlap) {
      ...
      if (std.dotProduct(dir, smallestAxis) < 0) smallestAxis = std.multVector(smallestAxis, -1);
    }
  }

  const mtv         = std.multVector(smallestAxis, smallestOverlap);
  const rawContacts = findContactPoints(polyA.vertices, polyB.vertices);
  const contacts    = transferContactsToA(rawContacts, polyA.vertices, std.multVector(smallestAxis, -1));
  ...
  if (inside || closestDistSq <= circle.radius * circle.radius) {
    ...
    if (inside) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (ov < minOverlap) { minOverlap = ov; axis = std.multVector(normal, -1); }
      }
      overlap = minOverlap;
    } else {
      ...
      if (dist > 1e-6) {
        axis = std.multVector(std.subVector(circle.basic.location, closestPoint), 1 / dist);
      } else {
        axis = new std.Vector(0, -1);
      }
    }
...
export function resolveCollisionPoly(
  polyA: Polygon, polyB: Polygon,
  contacts: std.Vector[], mtv: std.Vector,
): Forces {
  const slop      = 0.5;
  ...
  return { forceA: new std.Vector(0, 0), angForceA: 0, forceB: new std.Vector(0, 0), angForceB: 0 };
}
...
export function resolveCollisionCircle(
  ...
  cB.basic.isGrounded = isCircleGrounded(cB.basic.angVelocity, std.multVector(velocity_AB, -1), std.multVector(mtvN, -1));
